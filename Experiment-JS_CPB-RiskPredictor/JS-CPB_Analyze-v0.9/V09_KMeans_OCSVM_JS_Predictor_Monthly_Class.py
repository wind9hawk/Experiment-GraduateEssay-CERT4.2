...
# 5. 检查CERT4.2中三类Insiders的Recall与整体误报率FPR；
...
class KMeans_OCSVM_Predictor():
    def __init__(self, src_dir, dst_dir, month):
        ...
        self.JS_lst = []
        self.CERT42_Users_JSOrder = []
        for line_js in f_JS_lst:
            line_lst = line_js.strip('\n').strip(',').split(',')
            if line_lst[0] == 'user_id':
                continue
            self.CERT42_Users_JSOrder.append(line_lst[0])
            js_tmp = []
            # js_tmp中不包含用户字段
    ...
    def Auto_KMeans(self):
        ...
        i = 0
        while i < len(self.CERT42_Users_JSOrder):
            if self.CERT42_Users_JSOrder[i] not in self.Leave_Users:
        ...
        while i < len(self.CERT42_Users_ATFOrder):
            if self.CERT42_Users_ATFOrder[i] not in self.Leave_Users:
        ...
        print len(self.ATF_lst), '\n'
        for i in range(10):
            print i, self.CERT42_Users_ATFOrder[self.KMeans_Index_ATF[i]], '\n'
        ...
        while j < len(self.KMeans_Index_ATF):
            ...
            f_kmeans_pred.write(self.CERT42_Users_ATFOrder[user_index] + ',')
    ...
    def OCSVM_Predictor(self):
        ...
        while j < len(self.Test_Index):
            if self.CERT42_Users_ATFOrder[self.Test_Index[j]] in self.Insiders_1:
                self.Test_Insiders_1_Index.append(j)
                j += 1
                continue
            if self.CERT42_Users_ATFOrder[self.Test_Index[j]] in self.Insiders_2:
                self.Test_Insiders_2_Index.append(j)
                j += 1
                continue
            if self.CERT42_Users_ATFOrder[self.Test_Index[j]] in self.Insiders_3:
        ...
        while j < len(self.Test_Feats):
            risk_tmp =[]
            risk_tmp.append(self.CERT42_Users_ATFOrder[self.Test_Index[j]])
