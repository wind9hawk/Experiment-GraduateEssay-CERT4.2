...
WorkTime_Oneself_Path = Dst_Dir + '\\' + 'V09_CERT4.2_Users_WorkOn-Off_Time.csv'
# CERT5.2的LDAP数据
LDAP_Users_Path = sys.path[0] + '\\' + 'CERT4.2-LDAPUsers.csv'
# 读入内容行列表
...
f_Team_WorkTime = open(Dst_Dir + '\\' + 'V09_CERT4.2_Team_WorkTime.csv', 'w')
# 定义最终要保存的Team时间列表
Team_WorkTime_lst = []
...
# 定义Team_WorkTime文件
f_Team_WorkTime = open(Dst_Dir + '\\' + 'V09_CERT4.2_Team_WorkTime.csv', 'r')
f_Team_WT_lst = f_Team_WorkTime.readlines()
f_Team_WorkTime.close()

# 定义一个保存的CERT5.2用户上下班文件
f_Users_WorkTime = open(Dst_Dir + '\\' + 'V09_CERT4.2_Users_WorkOn-Off_Time_Team.csv', 'w')
