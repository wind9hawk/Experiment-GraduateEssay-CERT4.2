...
print '......<<<<<<开始确定CERT4.2中用户的上下班时间>>>>>>.....\n\n'


print '....<<<<准备数据源>>>>....\n\n'
Logon_Dir = os.path.dirname(sys.path[0]) + '\\' + 'CERT4.2_Logon_ByUsers_0.6'

#if os.path.exists(sys.path[0] + '\\' + 'JS-Risks_Analyze-0.7') == False:
#    os.mkdir(sys.path[0] + '\\' + 'JS-Risks_Analyze-0.7')
Dst_Dir = sys.path[0]

f_WorkTime = open(Dst_Dir + '\\' + 'V09_CERT4.2_Users_WorkOn-Off_Time.csv', 'w')

# 定义最终输出的用户上下班时间
# [user_id, WorkOn_Time. WorkOff_Time]
# CERT42_Users_WorkTime = []
