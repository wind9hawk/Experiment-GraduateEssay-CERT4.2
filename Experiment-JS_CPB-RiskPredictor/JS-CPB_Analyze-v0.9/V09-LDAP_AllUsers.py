# coding:utf-8
# 在进一步落实了CERT4.2中的层级结构后，尝试建立所有事业部的员工列表，计划：
# (1)先统计形成每个事业部的部门列表，形式为[BU-X, FU-Y, DPT-Z, TEAM-P]，先列出部门列表，按顺序排列；
# (2)然后重新遍历2009-12的初始LDAP（1000用户），将其中LDAP不全的用户归为Others列表，剩余的按照第一步得到的部门列表进行分别对应构成成员列表
# (3)由于比照的部门列表，因此可以用用户的LDAP字段在部门列表中进行index匹配确定位置，然后写入到对应的列表即可；
# (4)最后依据部门列表将所有用户成员列表输出到一个CSV文件；
# (5)本程序输出两个文件，一个是CERT4.2中所有部门列表，一个是所有成员归属的列表文件；

import os
import sys


print '全面分析CERT4.2中用户的LDAP列表归属，依据2009-12建立初始2000用户归属文件...\n'
# 首先需要定位用户初始LDAP位置
...
# 相比CERT5.2，CERT4.2的LDAP没有Project信息
print '写入用户部门列表LDAP到文件CERT-LDAP.csv\n'
f = open('CERT4.2-LDAP.csv', 'w')
f.write('LDAP on BU/FU/DPT/TEAM in CERT4.2\t')
f.write(str(len(LDAP)))
f.write('\n')
for ldap in LDAP:
    f.write(str(ldap))
    f.write('\n')
print 'CERT4.2-LDAP.csv写入完毕...\n'
f.close()
...
f = open('CERT4.2-LDAPUsers.csv', 'w')
f.write('CERT4.2 LDAP Users\n')
...
while i < len(LDAP_Users):
    ...
    f.write('\n')
    i += 1
print 'CERT4.2-LDAPUsers.csv写入完毕...\n'
f.close()
